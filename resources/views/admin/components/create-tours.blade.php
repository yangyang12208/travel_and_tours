...
<div class="container-fluid">
    ...
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        <h1 class="h3 mb-0 text-muted">Add Tours</h1>
    ...
    <div class="shadow-lg p-3 mb-5 bg-body rounded">
     ...
     <div class="row justify-content-center mb-3">
        <div class="col-md-5">
            <form action="{{ route('create_tours') }}" method="POST" enctype="multipart/form-data">
                @csrf
            <label for="">Destination Category</label>
           <select name="destination_category" id="" class="form-control" >
            <option value="" disabled selected> --- Select Category ---</option>
                @foreach ($data as $destination )
                    <option value="{{ $destination->destinations }}">{{ $destination->destinations }}</option>
                @endforeach
           </select>
        </div>
     </div>
        ...
        <div class="row justify-content-center">
            ...
            <div class="col-md-5">
                <div class="mb-3">
                    ...
                    <input type="number" value="{{ old('destination_day')}}"  name="destination_day"  class="@error('destination_day') text-danger is-invalid @enderror form-control" value=""  placeholder="number of days">
                </div>     
            </div>
        </div>
